...
void Camera::KeyMove(std::string direction, float deltaTime) {
	float velcocity = Speed * deltaTime;

	if (direction == "FORWARD") Position += Front * velcocity;
	if (direction == "BACKWARD") Position -= Front * velcocity;
	if (direction == "RIGHT") Position += Right * velcocity;
	if (direction == "LEFT") Position -= Right * velcocity;
}
