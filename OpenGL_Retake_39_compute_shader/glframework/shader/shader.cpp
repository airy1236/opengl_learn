...
ComputeShader::ComputeShader(const char* computePath) {
    ...
    CheckShaderError(compute, "COMPUTE");

    // shader Program
    programe = glCreateProgram();
    glAttachShader(programe, compute);
    glLinkProgram(programe);
    CheckShaderError(programe, "PROGRAM");
...
void ComputeShader::CheckShaderError(int target, const std::string mode) {
	int success;
	char infolog[1024];
	if (mode == "COMPILE") {
		glGetShaderiv(target, GL_COMPILE_STATUS, &success);
...
void ComputeShader::setVec2(const std::string& name, const glm::vec2& value) const {
    glUniform2fv(glGetUniformLocation(programe, name.c_str()), 1, &value[0]);
}
void ComputeShader::setVec2(const std::string& name, float x, float y) const {
    glUniform2f(glGetUniformLocation(programe, name.c_str()), x, y);
}

void ComputeShader::setVec3(const std::string& name, const glm::vec3& value) const {
    glUniform3fv(glGetUniformLocation(programe, name.c_str()), 1, &value[0]);
}
void ComputeShader::setVec3(const std::string& name, float x, float y, float z) const {
    glUniform3f(glGetUniformLocation(programe, name.c_str()), x, y, z);
}

void ComputeShader::setVec4(const std::string& name, const glm::vec4& value) const {
    glUniform4fv(glGetUniformLocation(programe, name.c_str()), 1, &value[0]);
}
void ComputeShader::setVec4(const std::string& name, float x, float y, float z, float w) {
    glUniform4f(glGetUniformLocation(programe, name.c_str()), x, y, z, w);
}

void ComputeShader::setMat2(const std::string& name, const glm::mat2& mat) const {
    glUniformMatrix2fv(glGetUniformLocation(programe, name.c_str()), 1, GL_FALSE, &mat[0][0]);
}

void ComputeShader::setMat3(const std::string& name, const glm::mat3& mat) const {
    glUniformMatrix3fv(glGetUniformLocation(programe, name.c_str()), 1, GL_FALSE, &mat[0][0]);
}

void ComputeShader::setMat4(const std::string& name, const glm::mat4& mat) const {
    glUniformMatrix4fv(glGetUniformLocation(programe, name.c_str()), 1, GL_FALSE, &mat[0][0]);
}
