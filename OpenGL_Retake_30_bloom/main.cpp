...
void processInput() {
    ...
    if (glfwGetKey(App->getWindow(), GLFW_KEY_SPACE) == GLFW_PRESS && !bloomKeyProcess) {
        bloom = !bloom;
        bloomKeyProcess = true;
    }

    if (glfwGetKey(App->getWindow(), GLFW_KEY_SPACE) == GLFW_PRESS) {
        bloomKeyProcess = false;
    }
    ...
    else if (glfwGetKey(App->getWindow(), GLFW_KEY_E) == GLFW_PRESS) {
        exposure += 0.001f;
    }
}
...
int main() {
    ...
    Shader shader("assets/shaders/vertex.vs", "assets/shaders/fragment.fs"),
        shaderLight("assets/shaders/light.vs", "assets/shaders/light.fs"),
        shaderBlur("assets/shaders/blur.vs", "assets/shaders/blur.fs"),
        shaderBloomFinal("assets/shaders/bloom.vs", "assets/shaders/bloom.fs");

    GLuint floorTexture = loadTexture("assets/textures/floor.png", true);
    GLuint containerTexture = loadTexture("assets/textures/diffuse_map.png", true);
    ...
    while (App->update()) {
        ...
        bool horizontal = true, first_interation = true;
        GLuint amount = 10;
        shaderBlur.begin();
        for (GLuint i = 0; i < amount; ++i) {
            glBindFramebuffer(GL_FRAMEBUFFER, pingpongFBO[horizontal]);
            shaderBlur.setBool("horizontal", horizontal);
            glBindTexture(GL_TEXTURE_2D, first_interation ? colorBuffers[1] : pingpongColorbuffer[!horizontal]);
            renderQuad();
            horizontal = !horizontal;
            if (first_interation) first_interation = false;
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        shaderBlur.end();
