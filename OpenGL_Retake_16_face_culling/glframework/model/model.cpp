...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene) {
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; ++i) {
		...
		else vertex.TexCoords = glm::vec3(0.0, 0.0, 0.0);
	...
	std::vector<mTexture> heightMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_height");
...
unsigned int Model::TextureFromFile(const char* path, const std::string& dirextory, bool gamma) {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, format, width, height, channels, format, GL_UNSIGNED_BYTE, data);
